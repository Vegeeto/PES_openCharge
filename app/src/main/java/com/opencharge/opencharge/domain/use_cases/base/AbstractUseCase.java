 ...
 * and finishing a usecase has mostly the same code throughout so that is why this class was created. Field methods
 * are declared volatile as we might use these methods from different threads (mainly from UI).
 * <p/>
 * For example, when an activity is getting destroyed then we should probably cancel a usecase
...
public abstract class AbstractUseCase implements UseCase {
     ...
     * developer should call the execute() method of a usecase to make sure the operation is done on a background thread.
     * <p/>
